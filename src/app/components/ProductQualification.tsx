...
export function ProductQualification() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const results = await api.searchSuppliers(formData);
      navigate("/suppliers", {
        state: {
          ...formData,
          suppliers: results,
      ...
      // Navigate anyway — SupplierResults will fall back to mock data
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-4">
              {step > 1 && (
                <button
                  ...
                  className="px-6 py-3 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors flex items-center gap-2"
                >
                  <ArrowLeft className="w-4 h-4" />
            ...
            {/* ── Error Message (OUTSIDE the buttons div, INSIDE the form) ── */}
            {error && (
              <p className="text-sm text-red-600 text-center mt-2 bg-red-50 border border-red-200 rounded-lg px-4 py-3">
                ⚠️ {error}
              </p>
            )}
