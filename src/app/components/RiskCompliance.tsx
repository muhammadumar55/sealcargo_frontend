  ...
  const goToReport = () => {
    navigate("/report", {
      state: {
        ...
        budget,
        riskScore,
        // Optional: pass supplier counts if you have them
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <p className="text-xs mt-1">
            {isLiveData ? (
              <span className="text-green-600 font-medium">
                ✅ Análisis dinámico para {supplierName} → {destination} (
                {productType})
              </span>
            ) : (
              <span className="text-yellow-600 font-medium">
                ⚠️ Análisis genérico — selecciona un proveedor para datos
                precisos
              </span>
            )}
          </p>
        ...
        {/* ── Risk Score Card ───────────────────────────────────────────────── */}
        <div
          className={`${riskColorClass} rounded-2xl shadow-xl p-8 mb-8 text-white`}
        >
          <div className="flex items-center justify-between">
            <div>
              <div className={`${riskTextClass} mb-2`}>
